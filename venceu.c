...
int main(){
	...
	SDL_Texture *proximafaseTexto = NULL;
	...
	if(SDL_Init(SDL_INIT_EVERYTHING) < 0){
		...
		if(window == NULL){
			...
			//Códigos para o botão proxima fase
			//surface com codificacao utf-8 para aceitar acento
			SDL_Surface *proximafaseSurface = TTF_RenderUTF8_Solid(font,"Próxima fase",color);
			
			proximafaseTexto = SDL_CreateTextureFromSurface(renderer,proximafaseSurface);
			
			SDL_Rect proximafaseRect;
			
			proximafaseRect.x = (LARGURA_JANELA/16) - (proximafaseSurface->w/12);
			proximafaseRect.y = (ALTURA_JANELA) - 300;//- (proximafase->h/2);
			proximafaseRect.w = proximafaseSurface->w;
			proximafaseRect.h = proximafaseSurface->h;

			SDL_QueryTexture(proximafaseTexto,NULL,NULL,&proximafaseRect.w,&proximafaseRect.h);
				...
				while(fechar != 1){
					...
					while(SDL_PollEvent(&event) != 0){ //Eventos de Teclado e Mouse
						...
						case SDL_MOUSEMOTION:
							...
							if(x >= proximafaseRect.x && x <= proximafaseRect.x+proximafaseRect.w && y >= proximafaseRect.y && y <= proximafaseRect.y+proximafaseRect.h){
									SDL_FreeSurface(proximafaseSurface);
									SDL_DestroyTexture(proximafaseTexto);
									proximafaseSurface = TTF_RenderUTF8_Solid(font,"Próxima fase",color2);
									proximafaseTexto = SDL_CreateTextureFromSurface(renderer,proximafaseSurface);
									pataRect.x = (LARGURA_JANELA/12) - (proximafaseSurface->w/12) - 40;
									pataRect.y = (ALTURA_JANELA + 7) - (proximafaseSurface->w/2 + proximafaseSurface->h/2);

									
							}else{
									SDL_FreeSurface(proximafaseSurface);
									SDL_DestroyTexture(proximafaseTexto);
									proximafaseSurface = TTF_RenderUTF8_Solid(font,"Próxima fase",color);
									proximafaseTexto = SDL_CreateTextureFromSurface(renderer,proximafaseSurface);
							}	
					...
					//proximafase
					SDL_RenderCopy(renderer,proximafaseTexto,NULL,&proximafaseRect);
					//Som
